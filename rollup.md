# learning-os — weekly rollup

Generated 2026-08-30 20:04 UTC. Pushed automatically by a systemd timer.

This file exists because a guardrail nobody else can see is not a guardrail. The
numbers below are not curated — if a week went badly, this is where it says so.

Targets are anonymised. What has to be uncomfortable here is the meta ratio and the
escape days, not which company — publishing that would be a disclosure, not a
guardrail.

## Guardrails

| guardrail | value | limit | status |
|---|---:|---:|---|
| meta_ratio | 0.00 | 0.12 | not measurable |
| loop_surface_index | — | 0.30 | not measurable |
| upstream_heartbeat | — | 21.00 | not measurable |
| escape_days | — | 2.00 | not measurable |
| contact_staleness | — | 90.00 | not measurable |
| rollup_freshness | 0.18 | 8.00 | ok |

Minutes logged in the last 28 days: **0**, of which **0** were spent on this tracker.

## Readiness

| target | score | weakest component |
|---|---:|---|
| target-a | 0.000 | round 1 of 3 |
| target-b | 0.000 | round 1 of 2 |
| en | 0.001 | round 1 of 4 |
| xx_fixture | 0.000 | round 1 of 2 |

## Gate

| company | state | attempts left | why |
|---|---|---:|---|
| target-a | no_go | 3 | Not the timing — the preconditions. The stopping rule says now — waiting no longer pays for itself. Blocked by all_rounds_above_floor, distinct_problems, five_minute_english, named_referrer, paid_mock_streak. Cannot yet measure: cold_first_pass_rate — judge these by hand until the instrumentation lands. |
| target-b | go | ∞ | The stopping rule says now — waiting no longer pays for itself. Every hard precondition is met. |

## Commits, last 14 days

- learning-os (the tracker): **1** days with commits
- technical repos: **0** days with commits
- not created yet: perf-gate, cpp-foundations, llm-kernels, perf-notes — until at least one exists, escape days cannot be measured at all

---

Self-assessment is weighted 0.00 everywhere in this system. Every number above
comes from evidence that survived contact with someone other than me.
